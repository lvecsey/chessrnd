...
int gameset(basegame_t *game) {
  ...
  
  {
    ...
    rank = 7;
    game->positions[rank*8+0] = setpos(BROOK, game->bl2.rooks+0);
    game->positions[rank*8+1] = setpos(BKNIGHT, game->bl2.knights+0);
    game->positions[rank*8+2] = setpos(BBISHOP, game->bl2.bishops+0);
    game->positions[rank*8+3] = setpos(BQUEEN, game->bl2.queens+0);
    game->positions[rank*8+4] = setpos(BKING, &game->bl2.king);
    game->positions[rank*8+5] = setpos(BBISHOP, game->bl2.bishops+1);
    game->positions[rank*8+6] = setpos(BKNIGHT, game->bl2.knights+1);
    game->positions[rank*8+7] = setpos(BROOK, game->bl2.rooks+1);
    ...
    rank = 0;
    game->positions[rank*8+0] = setpos(WROOK, game->wh2.rooks+0);
    game->positions[rank*8+1] = setpos(WKNIGHT, game->wh2.knights+0);
    game->positions[rank*8+2] = setpos(WBISHOP, game->wh2.bishops+0);
    game->positions[rank*8+3] = setpos(WQUEEN, game->wh2.queens+0);
    game->positions[rank*8+4] = setpos(WKING, &game->wh2.king);
    game->positions[rank*8+5] = setpos(WBISHOP, game->wh2.bishops+1);
    game->positions[rank*8+6] = setpos(WKNIGHT, game->wh2.knights+1);
    game->positions[rank*8+7] = setpos(WROOK, game->wh2.rooks+1);

    rank = 1;
    for (file = 0; file < 8; file++) {
      game->positions[rank*8+file] = setpos(WPAWN, game->wh2.pawns+file);
    }    
